...
为避免“只有脚本产物、缺少实现解读”，本技能产物强制分两层：

1. **Auto Layer（自动层，可覆盖）**
   - 典型产物：`docs/development/00-overview/codemaps/**`、`docs/_meta/codemap-summary.json`
   - 来源：脚本自动从代码结构提取（imports/exports/path grouping）
   - 允许脚本覆盖重写

2. **Manual Layer（人工层，不可脚本覆盖）**
   - 典型产物：实现现状、状态机说明、阈值解释、已知限制、调试与验证指南
   - 建议路径：`docs/development/30-modules/**`（按模块维护）
   - **禁止**由 `generate.ts` 覆盖

执行结论：脚本只负责“代码事实骨架”，不能替代“实现解读文档”。
...
4. **刷新 Auto Layer 索引**：更新 `README.md` 与 `docs/GUIDES/*`（若存在）。
5. **补齐 Manual Layer（强制）**：若命中核心模块变更，必须补实现解读（至少“现状/边界/限制”三块）。
6. **快速校验**：路径、链接、命令示例可执行性。
7. **交接治理**：输出变更清单与未验证项，交由 `documentation-maintainer` 门禁验收。

## 何时必须更新 Manual Layer（新增）

命中任一条件时，`doc-updater` 不得只生成 codemap：

- `src/features/**`、`src/lib/audio/**`、`src/lib/music/**` 出现行为变化（状态、阈值、算法、分支逻辑）。
- 类型契约变化（`src/types/**`）影响调用方理解。
- 用户明确要求“代码实际实现情况文档”。
...
默认采用“增量更新”，禁止每次无差别全量重写。

1. 若仓库可用 Git，先读取差异（`git diff` / `git status`）作为更新输入。
2. 仅对“变更代码文件 + 受影响邻接模块”更新 Manual Layer 文档。
3. 仅在以下场景执行全量重扫：
   - 大规模目录迁移/重命名
   - 入口架构变化（如 `main` / app root 重构）
   - 发布前全量审计
4. 输出中必须披露：
   - 本次差异范围（文件列表）
   - 文档映射关系（code -> docs）
